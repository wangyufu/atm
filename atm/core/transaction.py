...
def make_transaction(log_obj, account_data, tran_type, amount, **others):
    ...
    if tran_type in settings.TRANSACTION_TYPE:
        ...
        if 'type' in others:
            log_obj.debug("account:%s   action:%s    amount:%s   interest:%s   new_balance:%s" %
                          (account_data['id'], others['type'], amount, interest, new_balance))
        else:
            log_obj.info("account:%s   action:%s    amount:%s   interest:%s   new_balance:%s" %
                         (account_data['id'], tran_type, amount, interest, new_balance))
        return account_data
    else:
